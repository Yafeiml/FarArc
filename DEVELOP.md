...
应用语义版本的唯一来源是 `FarArc/AppVersion.cs`。`FarArc.csproj` 中的产品版本、程序集版本和文件版本必须与其同步；三段语义版本 `0.1.0` 对应四段程序集/文件版本 `0.1.0.0`。
...
- 发布只由 `v*` 标签触发；
- 标签必须与 `scripts/Get-Version.ps1` 输出完全一致，例如 `v0.1.0`；
