...
namespace FarArc.Service
{
    public class ProtocolConfigurationService
    {
        ...
        private void Load(string protocolFolderName)
        {
            ...
            // SSH_PRIVATE_KEY_PATH 改名为 FARARC_PRIVATE_KEY_PATH 2023年10月12日，TODO 一年后删除此代码
            var kys = new Dictionary<string, string>()
            {
                { "%SSH_PRIVATE_KEY_PATH%", "%FARARC_PRIVATE_KEY_PATH%" },
                { "%HOSTNAME%", "%FARARC_HOSTNAME%" },
                { "%PORT%", "%FARARC_PORT%" },
                { "%USERNAME%", "%FARARC_USERNAME%" },
                { "%PASSWORD%", "%FARARC_PASSWORD%" },
                { "%PRIVATE_KEY_PATH%", "%FARARC_PRIVATE_KEY_PATH%" },
            };
            ...
            if (c.Runners.Count == 0 || c.Runners.All(x => x is InternalDefaultRunner))
            {
                ...
                if (VNC.ProtocolName == protocolName)
                {
                    if (c.Runners.All(x => x.Name != "UltraVNC"))
                        c.Runners.Add(new ExternalRunner("UltraVNC", protocolName)
                        {
                            ...
                            Arguments = @"%FARARC_HOSTNAME%:%FARARC_PORT% -password %FARARC_PASSWORD%",
                    ...
                    if (c.Runners.All(x => x.Name != "TightVNC"))
                        c.Runners.Add(new ExternalRunner("TightVNC", protocolName)
                        {
                            ...
                            Arguments = @"%FARARC_HOSTNAME%::%FARARC_PORT% -password=%FARARC_PASSWORD% -scale=auto",
                            RunWithHosting = true,
                            EnvironmentVariables = new ObservableCollection<ExternalRunner.ObservableKvp<string, string>>(new[] { new ExternalRunner.ObservableKvp<string, string>("VNC_PASSWORD", "%FARARC_PASSWORD%") }),
                        });
                }
                if (SFTP.ProtocolName == protocolName)
                {
                    if (c.Runners.All(x => x.Name != "WinSCP"))
                        c.Runners.Add(new ExternalRunnerForSSH("WinSCP", protocolName)
                        {
                            ...
                            Arguments = @"sftp://%FARARC_USERNAME%:%FARARC_PASSWORD%@%FARARC_HOSTNAME%:%FARARC_PORT%",
                            ArgumentsForPrivateKey = @"sftp://%FARARC_USERNAME%@%FARARC_HOSTNAME%:%FARARC_PORT% /privatekey=%FARARC_PRIVATE_KEY_PATH%",
                        });
                }
                if (SSH.ProtocolName == protocolName)
                {
                    if (c.Runners.All(x => x.Name != "Putty"))
                        ...
                        {
                            ExePath = @"D:\PuTTY.exe",
                            Arguments = @"-ssh %FARARC_HOSTNAME% -P %FARARC_PORT% -l %FARARC_USERNAME% -pw %FARARC_PASSWORD% -%SSH_VERSION% -cmd ""%STARTUP_AUTO_COMMAND%""",
                            ArgumentsForPrivateKey = @"-w 1 new-tab --title ""%FARARC_HOSTNAME%"" --suppressApplicationTitle plink -ssh %FARARC_HOSTNAME% -P %FARARC_PORT% -%SSH_VERSION% -C -X -no-antispoof -l %FARARC_USERNAME% -i %FARARC_PRIVATE_KEY_PATH%",
                        });
                    if (c.Runners.All(x => x.Name != "Windows Terminal"))
                        c.Runners.Add(new ExternalRunnerForSSH("Windows Terminal", protocolName)
                        {
                            ExePath = @"wt.exe",
                            Arguments = @"-w 1 new-tab --title ""%FARARC_HOSTNAME%"" --suppressApplicationTitle plink -ssh %FARARC_HOSTNAME% -P %FARARC_PORT% -%SSH_VERSION% -C -X -no-antispoof -l %FARARC_USERNAME% -pw %FARARC_PASSWORD%",
                            ArgumentsForPrivateKey = @"-w 1 new-tab --title ""%FARARC_HOSTNAME%"" --suppressApplicationTitle plink -ssh %FARARC_HOSTNAME% -P %FARARC_PORT% -%SSH_VERSION% -C -X -no-antispoof -l %FARARC_USERNAME% -i %FARARC_PRIVATE_KEY_PATH%",
                        });
                }
            }
