...
namespace FarArc.Model.ProtocolRunner.Default
{
    public class KittyRunner : InternalExeRunner
    {
        ...
        public override void Install(string path = "")
        {
            ...
            FarArc.Utils.PuTTY.Model.Utils.Install("Resources/KiTTY/kitty_portable.exe", path);
        ...
        public override string GetExeArguments(ProtocolBase protocol)
        {
            ...
            if (p is SSH ssh)
            {
                ...
                //var template = $@" -load ""{this.GetSessionId()}"" %FARARC_HOSTNAME% -P %FARARC_PORT% -l %FARARC_USERNAME% -pw %FARARC_PASSWORD% -%SSH_VERSION% -cmd ""%STARTUP_AUTO_COMMAND%""";
