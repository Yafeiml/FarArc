...
Write-Host 'Building 1Remote 1.2.1 taskbar recovery v6'
...
$sourcePath = Join-Path $workspace 'Ui\View\Host\TabWindowView.taskbar_recovery_v6.cs'
if (-not (Test-Path $sourcePath)) {
    throw "Taskbar recovery v6 source file was not found: $sourcePath"
}
...
Write-Host 'Recovery-v6 source validation passed.'
...
$verifyBundle = Join-Path $env:RUNNER_TEMP 'recovery-v6-compiled-bundle'
$verifySource = Join-Path $env:RUNNER_TEMP 'TabWindowView.recovery-v6.decompiled.cs'
...
$requiredCompiled = @(
    ...
    'SIBJumpView',
    'V6NativePostMessage',
    'V6NativeGetAsyncKeyState'
)
foreach ($fragment in $requiredCompiled) {
    if (-not $compiled.Contains($fragment)) {
        throw "Compiled EXE is missing recovery-v6 fragment: $fragment"
    }
}
...
Write-Host 'Compiled EXE recovery-v6 verification passed.'

$sourceSha = (git -C $workspace rev-parse HEAD).Trim()
$readmePath = Join-Path $publishDir 'README-Taskbar-Recovery-V6.txt'
$readmeLines = @(
    '1Remote 1.2.1 - Windows 11 25H2 taskbar recovery v6',
    '',
    ...
    '',
    'What V6 changes:',
    '- Normal native SC_MINIMIZE has priority and cancels recovery immediately.',
    '- Begins native foreground observation after 110 ms; measured normal native delivery was at most 90.4 ms.',
    '- Takes over at 170 ms only after three stable self/taskbar samples and no foreign top-level foreground.',
    '- Recognizes StartAllBack SIBJumpView/SIB* windows as taskbar transaction surfaces.',
    '- Rejects right/middle-button taskbar interactions.',
    '- Posts the standard native WM_SYSCOMMAND / SC_MINIMIZE command.',
    '- Legitimate SC_RESTORE is untouched.',
...
$zipPath = Join-Path $workspace '1Remote-1.2.1-win-x64-Win11-25H2-taskbar-recovery-v6.zip'
if (Test-Path $zipPath) {
    Remove-Item $zipPath -Force
}
Compress-Archive -Path (Join-Path $publishDir '*') -DestinationPath $zipPath -CompressionLevel Optimal
$zipHash = (Get-FileHash $zipPath -Algorithm SHA256).Hash.ToLowerInvariant()
Write-Host "Recovery-v6 ZIP: $zipPath"
Write-Host "Recovery-v6 ZIP SHA-256: $zipHash"
