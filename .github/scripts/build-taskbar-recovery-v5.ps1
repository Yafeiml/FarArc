...
Write-Host 'Building 1Remote 1.2.1 taskbar recovery v5'
...
$sourcePath = Join-Path $workspace 'Ui\View\Host\TabWindowView.taskbar_recovery_v5.cs'
if (-not (Test-Path $sourcePath)) {
    throw "Taskbar recovery v5 source file was not found: $sourcePath"
}
...
foreach ($fragment in $requiredSource) {
    if (-not $source.Contains($fragment)) {
        throw "Required recovery-v5 source fragment is missing: $fragment"
    }
}
...
Write-Host 'Recovery-v5 source validation passed.'
...
$verifyBundle = Join-Path $env:RUNNER_TEMP 'recovery-v5-compiled-bundle'
$verifySource = Join-Path $env:RUNNER_TEMP 'TabWindowView.recovery-v5.decompiled.cs'
...
foreach ($fragment in $requiredCompiled) {
    if (-not $compiled.Contains($fragment)) {
        throw "Compiled EXE is missing recovery-v5 fragment: $fragment"
    }
}
...
Write-Host 'Compiled EXE recovery-v5 verification passed.'

$sourceSha = (git -C $workspace rev-parse HEAD).Trim()
$readmePath = Join-Path $publishDir 'README-Taskbar-Recovery-V5.txt'
$readmeLines = @(
    '1Remote 1.2.1 - Windows 11 25H2 taskbar recovery v5',
    '',
    ...
    '',
    'What V5 changes:',
    '- Uses a thread-pool native foreground watchdog; it does not depend on WPF IsActive or DispatcherTimer.',
    '- Cancels immediately when a foreign application becomes foreground.',
    '- Recovers when the same session HWND returns to foreground without native minimization.',
    '- Recovers when foreground remains 0 / taskbar shell for at least 700 ms with the session still visible and non-iconic.',
    '- Posts the standard native WM_SYSCOMMAND / SC_MINIMIZE command.',
    '- Normal SC_MINIMIZE and every legitimate SC_RESTORE remain untouched.',
    '- No AppUserModelID, ITaskbarList, ShowInTaskbar, owner, or style mutation.',
    '',
    'Trace:',
    '- Primary: application locality .logs\TaskbarRecoveryV5-<PID>-<timestamp>.log',
    '- Fallback: %TEMP%\1Remote-TaskbarRecoveryV5',
    '',
...
$zipPath = Join-Path $workspace '1Remote-1.2.1-win-x64-Win11-25H2-taskbar-recovery-v5.zip'
if (Test-Path $zipPath) {
    Remove-Item $zipPath -Force
}
Compress-Archive -Path (Join-Path $publishDir '*') -DestinationPath $zipPath -CompressionLevel Optimal
$zipHash = (Get-FileHash $zipPath -Algorithm SHA256).Hash.ToLowerInvariant()
Write-Host "Recovery-v5 ZIP: $zipPath"
Write-Host "Recovery-v5 ZIP SHA-256: $zipHash"
